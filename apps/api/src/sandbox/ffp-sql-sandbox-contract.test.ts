...
describe('1. package pin ffp-sql-sandbox@0.1.2', () => {
  it('is an exact production dependency', () => {
    ...
    assert.equal(pkg.dependencies?.['ffp-sql-sandbox'], '0.1.2');
    assert.equal(pkg.devDependencies?.['ffp-sql-sandbox'], undefined);
  });

  it('is locked at 0.1.2 in pnpm-lock.yaml', () => {
    const lockfile = readFileSync(
      path.join(API_ROOT, '../../pnpm-lock.yaml'),
      'utf8',
    );
    assert.match(lockfile, /ffp-sql-sandbox@0\.1\.2:/);
  });
});
