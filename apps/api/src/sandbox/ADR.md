...
Pinned dependency: `ffp-sql-sandbox@0.1.2`. Nest keeps `SandboxService.execute(sql, dataSource)` as the host facade. Docker container create/exec, password-in-`Env`, and the local SQL validator execution path live in the package (or are deleted), not in this adapter.
...
- Password is decrypted with `CryptoService` in this adapter and passed to `executeSql` as `connection.password`. ffp writes it to tmpfs. Container `Env` must not contain the password (`DB_PASS` / `PGPASSWORD` are forbidden here).
- Default runner image is package `DEFAULT_SANDBOX_IMAGE` (digest). `image` is passed only when `SANDBOX_IMAGE` is explicitly set (untrusted override).
